...
async function copyProductLink(produtoId, photoIndex = 0) {
  const link = getProductShareUrl(produtoId, photoIndex);
  try {
...
function buildShareMessage(produto, photoIndex = 0) {
  const link = getProductShareUrl(produto.id, photoIndex);
  const legenda = getLegenda(produto, photoIndex);
  const modelo = legenda ? ` — ${legenda}` : '';
  return `Olá! Confira este produto da DS A Fonte: ${produto.nome}${modelo}\n${link}`;
}

function shareProductLink(produto, photoIndex = 0) {
  const mensagem = buildShareMessage(produto, photoIndex);
  window.open(
...
function buildMensagem(produto, tamanho, variante, fotoUrl) {
  ...
  if (fotoUrl) msg += ` Foto de referência: ${fotoUrl}`;
  msg += ` Ver no site: ${getProductShareUrl(produto.id, modalState.photoIndex || 0)}`;
  ...
  return `
    <article class="product-card${cardExtras ? ' ' + cardExtras : ''}" data-id="${produto.id}" data-category="${produto.categoria}" data-group="${produto.grupo}">
      ...
      <div class="product-card__body">
        ...
        <button class="btn btn--outline btn--sm product-card__copy-link" type="button" data-copy-link="${produto.id}" aria-label="Copiar link do produto">
...
function bindProductCardEvents(container) {
  ...
  container.querySelectorAll('[data-copy-link]').forEach(btn => {
    btn.addEventListener('click', async () => {
      const produto = getProduto(btn.dataset.copyLink);
      if (!produto) return;
      const card = btn.closest('.product-card');
      const variantIndex = card?.querySelector('.variant-btn.active')?.dataset.variantIndex;
      const fotoIndex = variantIndex != null
        ? (getVariante(produto, Number(variantIndex))?.foto ?? Number(variantIndex))
        : 0;
      const ok = await copyProductLink(produto.id, fotoIndex);
      if (ok) {
...
function bindShareButtons(produto) {
  const copyBtn = document.getElementById('productModalCopyLink');
  const shareBtn = document.getElementById('productModalShareLink');

  if (copyBtn) {
    copyBtn.onclick = async () => {
      const ok = await copyProductLink(produto.id, modalState.photoIndex);
      if (ok) {
  ...
  if (shareBtn) {
    shareBtn.onclick = () => shareProductLink(produto, modalState.photoIndex);
  }
}
